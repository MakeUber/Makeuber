...
if(isset($_FILES["myfile"]))
{
	...
	$error =$_FILES["myfile"]["error"];
   {
	...
	$add_dvar = array('uniq' => $uniq, 'image' => $image_name, 'status' => '1', 'time' => time());
			...
			$sql_gal = "INSERT into pro_img(sort, $insert_q[0]) SELECT max(sort)+1, $insert_q[1] from dis_img"; 
			$fg = 'ad';
			mysql_query($sql_gal);
    	}
    	else
	...
	echo		$sql_gal = "INSERT into pro_img(sort, $insert_q[0]) SELECT max(sort)+1, $insert_q[1] from dis_img";
			$fg = 'ad';
			mysql_query($sql_gal);
