...
$uniq_id = random_generator(10);
 ...
 if($_SESSION['type'] <> ''){
	define('PATH_INFORMATION',$_SESSION['path']);
 }
...
// 
	echo '<pre>'; 
	 if ($user) {
		 $path_info = $_SESSION['path'];
		// print_r($_SESSION);
		// echo $path_info;die;
		 //unset($_SESSION['token']);die;
  ...
  $dvar['first_name'] = $user['given_name'];
	...
	$userfetch = mysql_fetch_array($exec_user);
		...
		user_login($id);
		print_r($_SESSION);
		echo PATH_INFORMATION;die;
		echo "<script>window.location = ".$path_info." </script>";
