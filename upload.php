...
if(isset($_FILES["myfile"]))
{
	...
	$error =$_FILES["myfile"]["error"];
   {
	...
	$add_dvar = array('uniq' => $uniq, 'image' => $image_name, 'status' => '1', 'time' => time());
				 list($insert_q[0], $insert_q[1]) = insert_query($dvar, $add_dvar );
			
			$sql_gal = "INSERT into dis_img(sort, $insert_q[0]) SELECT max(sort)+1, $insert_q[1] from dis_img"; 
			$fg = 'ad';
			mysql_query($sql_gal);
    	}
    	else
	...
	echo		$sql_gal = "INSERT into dis_img(sort, $insert_q[0]) SELECT max(sort)+1, $insert_q[1] from dis_img";
			$fg = 'ad';
			mysql_query($sql_gal);
