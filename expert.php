...
<div id="main" style="height:auto;">
	...
	<div class="col-sm-3" style="padding:10px;height:auto;">
		<div class="panel panel-default" style="background:#FFF;height:100%;margin-top:100px;margin-left:45px;">
			<div class="panel-heading">
				Filter By
				<a href="expert.php" class="pull-right" style="color:blue;">Clear Filters</a>
			</div>
	...
	<option value="0-5" <?php if($_GET['Review'] == '0-5'){ echo "selected='selected'";} ?>>0-5</option>
    <option value="6-10"  <?php if($_GET['Review'] == '6-10'){ echo "selected='selected'";} ?>>6-10</option>
    <option value="11-15"  <?php if($_GET['Review'] == '11-15'){ echo "selected='selected'";} ?>>11-15</option> 
    <option value="16-20"  <?php if($_GET['Review'] == '16-20'){ echo "selected='selected'";} ?>>16-20</option> 
    <option value="More than 20"  <?php if($_GET['Review'] == 'M20'){ echo "selected='selected'";} ?>>More than 20</option>  
				</select>
				<h4>City</h4>
	...
	{if($row_area['area'] <> '') { ?>
		...
		<div class="input-group col-sm-12 " style="float:left;">
         ...
         <br><br> <br> <br> 
				...
				<div class="list-group">
					<h4>Category</h4>
					<a href="expert.php" class="list-group-item active" style="background-color:#5E5E5E;border-color:#414344;">All
		...
		<div class="col-sm-12 content" style="width:70%;margin-left:25px;margin-top:84px;">
...
<?php
	if(isset($_POST['picSubmit'])){
		...
		if(mysql_num_rows($result) > 0){
			echo '<div class="col-sm-4" style="background:#eaeaea;margin-top:30px;padding:20px;">';
		$keys=1;
...
<span class="like<?php echo $row['id'];?>"><?php if($user_status <> 0 ){?><a  style="cursor:pointer" onClick="like(<?php echo $row['id'];?>,<?php echo  "'".$user_uniq."'"?>)">Like</a><?php }else{?><a  data-toggle="modal" data-target="#loginModal" onClick="like(<?php echo $row['id'];?>,<?php echo  "'".$user_uniq."'"?>)">Like</a> <?php }?></span> <?php }?>&nbsp;&nbsp;&nbsp;<a href="#">Comment</a></span>
        ...
        <?php }?>
				...
				<div style="background:#ccc;padding:5px;position:absolute;left:60%;bottom:0px;width:40%">
					<div style="float:left;"></div><div style="float:left;margin-left:10px;width:90%;">
    ...
    $query = "Select user.* from user left outer join review on review.designer_id=user.uniq_id where user.type='1' and user.status='1' and user.first_name like '$name_arr[0]%' group by user.id";
		...
		if(mysql_num_rows($result) > 0){
			...
			while($row = mysql_fetch_assoc($result)){
				...
				echo '<div class="col-sm-4" style="padding:20px;margin-top:10px;background:#fff;border:1px solid #c9b7b7;">
					<div class="cover">';
				$i = 1;
				while ($rows = mysql_fetch_assoc($res)) {
					...
					if($i == 3){
						...
						<img src="./img/'.$rows['image'].'" height="340px" width="300%" class="portfolio_pic">
						</div>';
						$i+=2;
						continue;
					}
...
?>
				...
				<div class="col-sm-12 profile_desc" id="<?php echo $row['uniq_id'];?>">
					...
					<div class="pull-right" style="background:linear-gradient(#fff,#eee);margin-top:20px;margin-right:30px;padding:0px;border:1px solid #ccc;border-radius:4px;box-shadow:0px 1px 1px rgba(0,0,0,0.08),0px 1px 0px rgba(255,255,255,0.8) inset">
						<ul class="stats">
							<li><span style="color:#428bca;font-size:18px;"><?php echo $num_rate; ?></span><br>reviews</li>
                            <?php if($user_status == 1 /*&& $fetch_user['type']==0*/){?>
							<li><a href="expert_portfolio.php?id=<?php echo $row['uniq_id'];?>" style="color:black;">Read<br> More</a></li>
                            <?php }else{?>
...
?>
			...
			<nav style="margin-left:0%;">
				<ul class="pagination">
					<li><a href="<?php echo $_SERVER['PHP_SELF'];?>?page=<?php if($page==1){ echo $pages;}else{ echo $pages-1;}?>&cat=<?php echo $_GET['cat'];?>&Review=<?php echo $_GET['Review'];?>&city=<?php echo $_GET['city'];?>&area=<?php echo $_GET['area'];?>"><span aria-hidden="true">&laquo;</span><span class="sr-only">Previous</span></a></li>
				<?php
					$pages  = ceil($num/8);
					//echo $pages;
					for($x=0;$x<$pages;$x++){
       ...
       <br/><br/><br/> <h1 align="center">No result Found</h1>
		...
		if(mysql_num_rows($result) > 0){
			...
			while($row = mysql_fetch_assoc($result)){
				...
				echo '<div class="col-sm-5" style="padding:20px;margin-top:10px;background:#fff;border:1px solid #c9b7b7;">'; 
					//'//<div class="cover">';
				$i = 1;
				/*while ($rows = mysql_fetch_assoc($res)) {
					//print_r($rows);
				...
				echo '<div class="col-sm-4 wide" style="padding:0px;">
						<img src="./img/'.$rows['image'].'" height="340px" width="300%" class="portfolio_pic">'; //</div>' 
...
?><div id="<?php echo $row['uniq_id'];?>"></div>
				...
				<a href="expert_portfolio.php?id=<?php echo $row['uniq_id'];?>">
			        <img src="./img/<?php if($row['image'] <> ''){ echo $row['image'];}else{ echo 'no_img.jpg';}?>?>" class="profile_pic" style="float:left;display:block;margin-top:-20px;margin-left:0px;">
				</a>	
					<span> &nbsp; &nbsp;ID : <?php echo $row ['id'] ; ?>  </span> 
					<div style="float:left;padding:10px;font-size:13px;"> 
					...
					<div class="pull-right" style="background:linear-gradient(#fff,#eee);margin-top:10px;margin-right:30px;padding:0px;border:1px solid #ccc;border-radius:4px;box-shadow:0px 1px 1px rgba(0,0,0,0.08),0px 1px 0px rgba(255,255,255,0.8) inset">
							<ul class="stats">
							<li><span style="color:#428bca;font-size:18px;"><?php echo $num_rate; ?></span><br>reviews</li>
                            <?php if($user_status == 1 /*&& $fetch_user['type']==0*/){?>
							<li><a href="expert_portfolio.php?id=<?php echo $row['uniq_id'];?>" style="color:black;">Read<br> More</a></li>
