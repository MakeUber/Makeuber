...
$page_child = 'add_edit_user_project.php';				// table name for query
...
if($_GET['do'] == 'delete' && ctype_digit($_GET['id'])){
	$id = $_GET['id'];
	$sql = "SELECT * from $base_tabl where id='$id'";
	$exec = mysql_query($sql);
	$fetch_exe=mysql_fetch_assoc($exe);
	$sql2 = "SELECT * from $tab2 where uniq='".$fetch_exe['project_id']."'";
	$exec2 = mysql_query($sql2);
	while($fetch_img=mysql_fetch_assoc($exe2)){
		unlink('img/'.$fetch_img['image']);
				}
	
	@unlink('../img/'.$fetch_exe['project_image']);
	@unlink('../img/'.$fetch_exe['floor_plan']);
	
	$sql = "DELETE from $base_tabl where id='$id'";
	$sql3 = "DELETE from $tab2 where uniq='".$fetch_exe['project_id']."'";
	 
	if(mysql_query($sql) and mysql_query($sql3)){
		$flag['d'] = $item;
	}
	else{
		$flag['q'] = 'r';
...
$sqlq = $sql1;
$sql2 = $sql1." ORDER BY sort DESC LIMIT $stcom , $perpage";
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>?1=1<?php echo $q_string_pg; ?>" method="post" name="<?php echo $item; ?>">
          ...
          <table width="100%" cellspacing="0" cellpadding="0"  class="maintbl" >
            <tr class="fstrow"  >
              ...
              <td align="left" width="20%">Project Name</td>
              <td align="left" width="15%">Eamil</td>
              <td align="center" width="15%">Phone Number </td>
            ...
            <tr class="scndrow <?php echo $z%2 ? '' : 'alternate';?>">
              ...
              <td align="left"><?php echo $fetch['apartment'];?></td>
              <td align="center"><div class="n_1"><?php echo $fetch['email']; ?></div></td>
              <td align="center"><div class="n_1"><?php echo $fetch['phone']; ?></div></td>
            <!--  <td align="center"><a href="sort_gen.php?type=name&tab=<?php echo $tabl; ?>&height=500&width=600&rel=<?php echo $fetch['id']; ?>" class="thickbox"><img src="images/sort-neither.png" title="Sort" width="13" height="16" /></a></td>-->
              ...
              <td align="center">
              	<a style="margin:0px 10px;" href="view_user_project.php?do=view&id=<?php echo $fetch['id'].$q_string_pg; ?>" title="View"> <img src="images/view.png" /> </a>
