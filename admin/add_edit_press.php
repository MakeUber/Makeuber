...
if($_GET['do'] == 'edit'){
	...
	$dvar['link'] = $fetch['link'];
	$image = $fetch['image'];

}

if($_POST['submitbut'] == 'Save'){
	$dvar['link'] = $_POST['link'];
	
	if($dvar['link'] == ''){$flag[1] = 'r';}
	...
	if(!empty($flag)){
		...
		if (substr($dvar['link'], 0, 7) <> 'http://') {
			$dvar['link'] = 'http://'.$dvar['link'];
		}
		...
		if($_GET['do'] == 'edit'){
			...
			$add_dvar = array( 'id' => $id,'image'=>$image_name, 'status' => '1', 'time' => time());
			$remove_dvar = array('image_delete');
      ...
      <div class="form5">
        <form id="add" name="add" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?1=1<?php echo $q_string; ?>" enctype="multipart/form-data">
            <table width="1150" border="0" cellpadding="5">
               ...
               <tr>                
                  <td align="right" class="label_form">Link: </td>
                  <td><input name="link" type="text" class="input_text" value="<?php echo $dvar['link'];?>"/><br class="clear" /></td>
              </tr>
                  <tr>                
