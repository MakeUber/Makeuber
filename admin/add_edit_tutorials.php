...
if($_POST['submitbut'] == 'Save'){
	...
	//counting no of description already present 
	$sql_count = "select count(*) from files where relation='".$fetch['uniq']."'";
	$exec_count = mysql_query($sql_count);
	list($num_count)= mysql_fetch_row($exec_count);
	//echo $num_count;die;
...
//			$change_dvar = array('status' => '1');
		...
		if(mysql_query($sql)){
			...
			if($_GET['do'] == 'edit'){ 
				...
				if($_FILES[pic][name] != ''){
					$uniq_id = multiple_files('pic', ROOT_DIR.'/file/', 'file/', $dvar['uniq'], $desc1, $_SERVER['REMOTE_ADDR'], 'doc_name', $c);
				}
			}
			$flag[$fg] = $item;
		}
...
if($flag[$fg] <> '' || $flag['d'] <> ''){
?>
<meta http-equiv="refresh" content="2; URL=<?php echo $page_parent."?5=5".$q_string; ?>">
<?php } ?>
<style type="text/css">
.edit2 {
	width: 245px;
                 ...
                 if(counter1 >= limit1){
                  alert("Error: Maximium 10 descriptions can upload ");
					 ...
					 var newdiv1 = document.createElement('div');
					  newdiv1.innerHTML = "<div style='padding-top: 5px;padding-right: 14px;float: left;width: 600px;>Description" + (counter1)+":&nbsp;</div>" + "<div style='width: 203px;margin-left: -623px;float: left;'><textarea  name='description[]' id='description' class='edit2' style='margin-left: 114px;'><?php echo $desc1 ;?></textarea> </div> <br class='clear'>";
