...
if($_POST['submitbut'] == 'Save'){
	...
	if($dvar['area'] == ''){$flag[60] = 'r';}
	else if($dvar['city'] == ''){$flag[59] = 'r';}
      ...
      <div class="form5">
        <form id="add" name="add" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?1=1<?php echo $q_string; ?>" enctype="multipart/form-data">
            <table width="1150" border="0" cellpadding="5">
             <tr>                
                  <td align="right" class="label_form">Select City: </td>
                  <td>
                  <select  class="input_text select_text" name="city">
    <option value="">City</option>
    <?php while ($cat_row = mysql_fetch_assoc($cat)) {?>
   
                    <option value="<?php echo $cat_row['id'];?>" <?php if($dvar['city']==$cat_row['id']){ echo "selected='selected'";} ?>><?php echo $cat_row['city'];?></option>
                     <?php }?>
    </select>
