...
$item = 'Project';				// table name for query
$page_child = 'add_edit_project.php';				// table name for query
...
$select = "$base_tabl.* ";
$from = "  $base_tabl where 1=1 and project_id=''";
...
if($_GET['search'] == '' || $_GET['search'] == 'search'){
	...
	$sql1 = "SELECT $select from $from AND $base_tabl.project_name like '%$search_txt_db%'"; 
}
...
if($_POST['Delete'] == 'Delete Selected'){
	...
	if($arr_num > 0){
		$sql_s = "SELECT project_image from $base_tabl where id in (";
...
if($_GET['do'] == 'delete' && ctype_digit($_GET['id'])){
	$id = $_GET['id'];
	$sql = "SELECT project_image from $base_tabl where id='$id'";
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>?1=1<?php echo $q_string_pg; ?>" method="post" name="<?php echo $item; ?>">
          ...
          <table width="100%" cellspacing="0" cellpadding="0"  class="maintbl" >
            <tr class="fstrow"  >
              ...
              <td align="center" width="15%"> Designer Name </td>
            ...
            <?php
                ...
                while($fetch = mysql_fetch_assoc($exec)){
					 $user="select * from user where uniq_id ='".$fetch['user_id']."'";
					 $sql_usr=mysql_query($user);
					 $fetch_usr=mysql_fetch_assoc($sql_usr);
					
            ?>
            <tr class="scndrow <?php echo $z%2 ? '' : 'alternate';?>">
              <td><input type='checkbox' name='chk[]' value='<?php echo $fetch['id'] ?>' id='checkme<?php echo $z; ?>' /></td >
              <td align="left"><div class="n_1"><img height="98px" width="98px" src="<?php if(empty($fetch['project_image'])){
	echo 'images/1.png';
	}else{
	      echo '../img/'.$fetch["project_image"].''; 
		  } ?>"/>
          </div></td>
              <td align="left"><div class="n_1"><?php echo $fetch['project_name']; ?></div></td>
              <td align="left"><div class="n_1"><?php echo $fetch_usr['first_name']; ?>&nbsp;<?php echo $fetch_usr['last_name']; ?></div></td>
              <td align="center"><a href="sort_project.php?type=page&tabl=<?php echo $tabl; ?>&height=500&width=600&rel=<?php echo $fetch_usr['uniq_id']; ?>" class="thickbox"><img src="images/sort-neither.png" title="Sort" width="13" height="16" /></a></td>
              ...
              <td align="center">
              	<a style="margin:0px 10px;" href="view_project.php?do=view&id=<?php echo $fetch['id'].$q_string_pg; ?>" title="View"> <img src="images/view.png" /> </a>
