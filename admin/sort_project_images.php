...
if($relation != ''){
	$sql = "SELECT * from $tabl where  images_id='".$images_id."' order by sort ASC";
}
else{
	$sql = "SELECT * from $tabl where images_id='".$images_id."' order by sort ASC";
}
...
<ul id="test-list">
	...
	while($fetch1 = mysql_fetch_assoc($exec)){
		echo '<li id="listItem_'.$fetch1[id].'" style="padding:2px;"><img src="images/arrow.png" alt="move" width="16" height="16" class="handle" /> <img src="../img/'.$fetch1[image].'" width="30px" height="25px"/></a></li>';
		$i++;
	}
