...
if($_GET['do'] == 'edit'){
	$sql = "SELECT  $tabl.*,files.* from $tabl left outer join files on $tabl.uniq=files.relation where $tabl.id='".$id."'";
	$sql = "SELECT * from $tabl where id='".$id."'";
	$exec = mysql_query($sql) or die(mysql_error());
	$numq = mysql_num_rows($exec);
	$fetch = mysql_fetch_assoc($exec);
	$dvar['title'] = $fetch['title'];
	$dvar['relation'] = $fetch['relation'];
	$dvar['desktop'] = $fetch['desktop'];
...
if($_POST['submitbut'] == 'Save'){
	...
	$dvar['relation'] = $_POST['relation'];
	$dvar['desktop'] = $_POST['desktop'];
	$fid = $_POST['fid'];
	$tool = $_POST['tool'];
	$pic = $_FILES['pic']['name'];
	...
	if(($_POST['counter'] <> '') && $description <> ''){
		 $new_count = $_POST['counter'];
	}
	else if($description <> ''){
		  $new_count = count($description);
	}
	
	 $pre_count =  $numq;
	 $total_count = $pre_count+$new_count;
	//print_r($arr_count);
	//echo '<pre>';
	//print_r($pic);
//	print_r($description);
...
//			$change_dvar = array('status' => '1');
		...
		if(mysql_query($sql)){
			...
			if($_GET['do'] == 'edit'){ 
					...
					//	update ids
					$file_id[$j] = $fetch['id'];
					//make array of multiple images
					array_push($img, $fetch['thumb']);
					array_push($img1, $fetch['thumb_big']);
					array_push($img2, $fetch['location']);
					array_push($img3, $fetch['file_name']);
					...
					if($file == '1'){
						unlink('../'.$fetch['thumb']);   ///unlink thumb
					...
					if(!empty($flag)){
						...
						for($c=1;$c<=$total_count;$c++){
							...
							if($c <= $pre_count){
									if($file == '1'){ 
									$thumb =  $path1;
									//$sql_up = "update files set thumb='".'thumb/'.$image_name."',thumb_big='".'thumb_big/'.$image_name."',location='".'file/'.$image_name."', file_name='".'file/'.$image_name."',description='".$fetch['description']."' where id='".$fetch['id']."'"; 
										 foreach($description as $key => $value){
										 $c_id = mysql_real_escape_string($file_id[$key]);
										 $sql_up = "update files set thumb='".$img[$key]."',thumb_big='".$img1[$key]."',location='".$img2[$key]."', file_name='".$img3[$key]."',description='".$value."' where id='".$c_id."'";  
									...
									if($file <> '1'){//if no changes the fetch data and update as it is
										foreach($description as $key => $value){
											//print_r($description);
										$c_id = mysql_real_escape_string($file_id[$key]);
										//$final_image_name =  mysql_real_escape_string($img[$key]);
										//echo '<pre>';
									 $sql_file = "update files set thumb='".$img[$key]."',thumb_big='".$img1[$key]."',location='".$img2[$key]."', file_name='".$img3[$key]."',description='".$value."' where id='".$c_id."'"; 
				...
				if($_FILES['pic']['name'] != '' && $desc != ''){
					foreach($pic as $k1 => $v1){ 
					$pic_name = mysql_real_escape_string($v1);
					$desc1 = $desc[$k1];
					$description = mysql_real_escape_string($desc1); 
					$uniq_id = multiple_files('pic', ROOT_DIR.'/file/', 'file/', $uniq, $description, $_SERVER['REMOTE_ADDR'], 'doc_name');
...
function check_validation(){
   ...
   var tool = $('#tool').val();
    ...
    if(title == '') {
		...
		var error = "Error:Please Enter tool used";
		$('#tool').focus();
	}
    if(error != "" && error != null){
		$('#error').html(error);
    ...
    <div class="content">
      ...
      <div class="form5">
        <form id="add" name="add" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?5=5<?php echo $q_string; ?>" enctype="multipart/form-data" onsubmit="return check_validation();">
          <table width="100%" border="0" cellpadding="5" border="2" bordercolor="#000000">
            <tr>
            ...
            <tr>
             ...
             <tr>
              ...
              <?php while($fetch_cat = mysql_fetch_assoc($exec_cat)){
				  ?>
              <option value="<?php echo $fetch_cat['id']?>" <?php if($fetch_cat['id'] == $dvar['relation']){ echo "selected='selected'";}?> ><?php echo $fetch_cat['name'];?></option>
              <?php }?>
              </select>
                <br class="clear" /></td>
            </tr>
            ...
            <div class="view2"><img src="<?php if($image <> ''){echo '../pics/'.$image;}else{echo 'images/1.png';} ?>" alt="images/1" width="98" height="98" /></div></td>
         </tr>
            <tr>
              <td align="right" class="label_form">Show on Desktop: </td>
              <td><input name="desktop"  type="checkbox" value="1" <?php if($dvar['desktop'] == '1'){echo 'checked="checked"';} ?>/>
            ...
            <tr>
              ...
              <td><div class="pic"> <img src="<?php if($fetch_field['location'] <> ''){echo "../".$fetch_field['location'];}else{echo 'images/1.png';} ?>" alt="images/1" width="98" height="98" /></div>
                <div class="edit1">
                <input type="hidden" name="fid[]" value="<?php echo $fetch_field['id'];?>" />
                  <input  name="pic[]"  type="hidden" value='<?php echo $fetch_field['location']; ?>' class='edit1'/>
          ...
          <tr>
              ...
              <td>
                  ...
                  <div style="float:left; padding-top:18px;">Description :</div><div style="float:left;"><textarea name="description[]" class="edit2"><?php echo $fetch_field ['description'];?> </textarea></div>
              </td>
            </tr>
        	 <?php  */}?>
                 ...
                 if(counter >= limit){
                  alert("Error: Maximium 10 images upload ");
                    ...
                    var newdiv = document.createElement('div');
                     newdiv.innerHTML = "<div style='margin-left: 206px;padding-top: 5px;padding-right: 14px;float: left;width: 600px;' >Image" + (counter)+":&nbsp;</div>" + "<div style='width: 203px;margin-left: -623px;float: left;'><input type='file' id='location' name='pic[]'  class='edit2' style='margin-left: 114px;padding: 10px;'></div> <br class='clear'>";
                 ...
                 if(counter1 >= limit1){
                  alert("Error: Maximium 10 descriptions can upload ");
					 ...
					 var newdiv1 = document.createElement('div');
					  newdiv1.innerHTML = "<div style='padding-top: 5px;padding-right: 14px;float: left;width: 600px;>Description" + (counter1)+":&nbsp;</div>" + "<div style='width: 203px;margin-left: -623px;float: left;'><textarea  name='description[]' id='description' class='edit2' style='margin-left: 114px;'></textarea> </div> <br class='clear'>";
          ...
          <input type="hidden" name="counter" id="counter1" value="" />
          ...
          <table width="50%" border="0" cellpadding="5" >
           <tr>
              <td></td>
              <td><div class="btn">
             <!--  <input  type="button" name="add" value="Add Tool" id="tb_clone_click" class="button" > 
                 ...
                 <input class='button' name='submitbut' value='Save' type='submit' />
                  ...
                  <a class="a_button" href="<?php echo $page_parent."?5=5".$q_string; ?>">Cancel</a> </div>
                  </td>
            </tr>
          </table>
        </form>
