...
if($_POST['submitbut'] == 'Save'){
	...
	$dvar['user_id'] = $_POST['user_id'];	
	$dvar['description'] = $_POST['description'];
	if($dvar['project_name'] == ''){$flag[35] = 'r';}
	else if($dvar['description'] == ''){$flag[31] = 'r';}
		else if($dvar['user_id'] == ''){$flag[34] = 'r';}
	...
	if(!empty($flag)){
		...
		if($_GET['do'] == 'edit'){
			...
			$add_dvar = array( 'id' => $id, 'status' => '1', 'time' => time(), 'project_image' => $image_name);
			$remove_dvar = array('image_delete');
  ...
  <div class="container">
	...
	echo "<img src='images/1.png' />";
	      ...
	      echo "<img src='../img/".$dvar['project_image']."' width='98px' height='98px'/>"; 
              ...
              <tr>                
                  <td align="right" class="label_form">Select Expert: </td>
                  <td><select class="input_text" name="user_id"><option>Select Expert</option> 
                  <?php
				  $user="Select * from user where type='1'";
				  $query=mysql_query($user);
				   while($uniq_id=mysql_fetch_assoc($query)){
	                ?>
                    <option value="<?php echo $uniq_id['uniq_id']; ?>" <?php if($dvar['user_id'] == $uniq_id['uniq_id']){ echo "selected='seleccted'";} ?>><?php echo $uniq_id['first_name']."&nbsp;".$uniq_id['last_name'];?></option>
                    <?php
				   }
				   ?>
               ...
               <tr>                
                  <td align="right" class="label_form">Project Name: </td>
              ...
              <tr>                
                  ...
                  <td> <textarea style="width:350px; height:100px; border-radius:5px; border-style:solid; border-width:1px; border-color:#CCC" name="description" id="description" ><?php echo $dvar['description'];?></textarea><br class="clear" /></td>
              </tr>           
           <tr>
                <td>&nbsp;</td>
                <td>
