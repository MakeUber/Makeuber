...
$query_string = array("search" => $_GET['search'],"do" => $_GET['do'], "id" => $_GET['id']);
...
//			$change_dvar = array('status' => '1');
		...
		if(mysql_query($sql)){
			...
			if($_GET['do'] == 'edit'){ 
					...
					if($validate[0] <> '1'){$flag['file'] = $validate[0];}
					else if($validate[1] <> ''){$file = '1';$ext = $validate[2];}
					if($file == '1'){
      ...
      <div class="form5">
        <form id="add" name="add" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?5=5<?php echo $q_string; ?>" enctype="multipart/form-data" onsubmit="return check_validation();">
          <table width="100%" border="0" cellpadding="5" border="2" bordercolor="#000000">
            ...
            <?php $i++;} } else{?>
            <tr>
              <td align="right" valign="top" class="label_form">Image(1): </td>
              <td>
                  <div style="float:left;"><input   name="pic[]" type="file" value='<?php echo $fetch_field['pic']; ?>' /></div>
                  <div style="float:left; padding-top:18px;">Description(1) :</div>
                  <div style="float:left;"><textarea   class="edit2"><?php echo $fetch_field['description'];?></textarea></div>
              </td>
            </tr>
        	 <?php }?>
            ...
            <tr class="tr_clone">
                  <td align="right" class="label_form">Tool Used(1) :</td>
                  <td><input  id="tool" name="tool[]" type="text" class="input_text" value=""/><br class="clear" /></td><br class="clear" />
		  ...
		  <?php } else{ $j=1; while($fetch_meta = mysql_fetch_assoc($exec_meta)){
            ...
            <tr class="tr_clone" style="border:1px solid red;">
                  ...
                  <td><input id="tool" name="tool[]" type="text" class="input_text" value="<?php echo $fetch_meta['tool'];?>"/><br class="clear" /></td><br class="clear" />
