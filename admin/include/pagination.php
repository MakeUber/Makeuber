...
else{$ps="<a href='$namep?page=$prev&order=$order&field=$field&cust_img=$cust_img&search=$search_txt$p_var' title='previous'>&laquo; Prev</a>";}
if($page==$pg_n){$nx="<span class='disabled_tnt_pagination'>Next &raquo;</span>";}
else{$nx="<a href='$namep?page=$nex&order=$order&field=$field&cust_img=$cust_img&search=$search_txt$p_var' title='next'>Next &raquo;</a>";}
	...
	$k='1';	$l='0';		$cnt = count($arr);
		while($l < $cnt){
			...
			else{$pgs[$l]="<a href='$namep?page=$arr[$l]&order=$order&field=$field&cust_img=$cust_img&search=$search_txt$p_var'>$arr[$l]</a>";}
			$k++; 
			$l++;
		}
