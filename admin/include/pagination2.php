...
if($stcom >= $num && $num <> 0){ header("Location: $namep?page=1&id=$id");}
$z = $num / $perpage;
$pg_n=(int)$z;
	if($pg_n<>$z){
	$pg_n++;
...
else{$ps="<a href='$namep?page=$prev&id=$id&order=$order&field=$field&search=$search_txt$p_var' title='previous'>&laquo; Prev</a>";}
if($page==$pg_n){$nx="<span class='disabled_tnt_pagination'>Next &raquo;</span>";}
else{$nx="<a href='$namep?page=$nex&id=$id&order=$order&field=$field&search=$search_txt$p_var' title='next'>Next &raquo;</a>";}
	...
	$k='1';	$l='0';		$cnt = count($arr);
		while($l < $cnt){
			...
			else{$pgs[$l]="<a href='$namep?page=$arr[$l]&id=$id&order=$order&field=$field&search=$search_txt$p_var'>$arr[$l]</a>";}
			$k++; 
			$l++;
		}
